...
public class AVLTreeProgram {
  ...
  private int[] getNumbersInput(Scanner scannerInput) {
    while (true) {
      ...
      if (userInputLine.isEmpty()) {
        System.err.println(
            "Fehler: Die Eingabe darf nicht leer sein. Bitte versuchen Sie es erneut.\n");
        continue;
      }
      ...
      for (int splitIdx = 0; splitIdx < numberStrings.length; splitIdx++) {
        ...
        try {
          ...
          System.err.println(
              "Fehler: '"
                  + trimmedCandidate
                  + "' ist keine gültige Ganzzahl. Bitte versuchen Sie es erneut.\n");
      ...
      if (!foundParseError) {
        if (parsedNumbers.isEmpty()) {
          System.err.println(
              "Fehler: Keine gültigen Zahlen gefunden. Bitte versuchen Sie es erneut.\n");
          continue;
        }
   ...
   *
   * @param scanner Scanner instance
   * @return validated traversal order string, null if user wants to exit, or "NEW_TREE" if user
   *     wants new input
   */
  private String getTraversalOrderInput(Scanner scannerForTraversalType) {
    ...
    while (true) {
      ...
      if (userChoice.equals("5")
          || userChoice.equals("neuer baum")
          || userChoice.equals("neue eingabe")
          || userChoice.equals("new")) {
        return "NEW_TREE";
      }
      ...
      if (matchesValidType) {
        ...
        System.err.println(
            "Erlaubte Werte sind: 1-6, preorder, inorder, postorder, levelorder, neuer Baum, oder beenden");
